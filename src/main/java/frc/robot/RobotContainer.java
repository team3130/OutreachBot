...
//  }
        ...
        if (m_chassis.getJoystickName().equals("Logitech Extreme 3D")){
            ...
            new JoystickButton(m_Gamepad, Constants.XBOXButtons.A).whileTrue(new Red(ledSubsystem));
            new JoystickButton(m_Gamepad, Constants.XBOXButtons.B).whileTrue(new Green(ledSubsystem));
            new JoystickButton(m_Gamepad, Constants.XBOXButtons.X).whileTrue(new Blue(ledSubsystem));
            new JoystickButton(m_Gamepad, Constants.XBOXButtons.Y).whileTrue(new Custom(ledSubsystem));
            new POVButton(m_Gamepad, Constants.XBOXButtons.LST_POV_N).whileTrue(new MovingRainbow(ledSubsystem));
        }
    }
    ...
    public void vomitShuffleBoardData() {
        ...
        ShuffleboardTab led = Shuffleboard.getTab("LEDSubsystem");
        led.add(ledSubsystem);
    }
