...
public class Navx {
    ...
    public static double getAngle() {
        if (m_bNavXPresent) {
            return Math.IEEEremainder((m_navX.getAngle() + 360) * (Constants.Chassis.kNavxReversed ? -1.0 : 1.0), 720);
        }
        return -1;
    }
    ...
    public static double getRate() {
        if (m_bNavXPresent) return m_navX.getRate() * (Constants.Chassis.kNavxReversed ? -1.0 : 1.0);
        return -1;
    }
